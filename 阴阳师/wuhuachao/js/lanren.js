	...
	function slide(slideMode){//轮播方法		
		if (isMoving==false){
			...
			if(slideMode){			//slideMode为1(true)，执行slide(1)，上一张
				...
				hidden.stop(true,true).animate({width:"580px",height:"300",top:"50px",left:"-75px",opacity:1},timer);
				curr.stop(true,true).animate({width:"580px",height:"300px",top:"50px",left:"695px",opacity:1},timer);
				...
				prev.stop(true,true).animate({width:"1000px",height:"400px",left:"110px",top:0,opacity:1},timer,function(){
				...
				hidden.stop(true,true).animate({width:"580px",height:"300px",top:"50px",left:"695px",opacity:1},timer);
				curr.stop(true,true).animate({width:"580px",height:"300px",top:"50px",left:"-75px",opacity:1},timer);
				//next  -->  curr     next中的图片li轮换到curr的位置      其他一次轮换
				next.find("span").css("opacity",0);
				$(".main_banner_box li").find("p").css({"bottom":"-50px"});//所有标题隐藏
				next.stop(true,true).animate({width:"1000px",height:"400px",left:"110px",top:0,opacity:1},timer,function(){
					$(this).find("p").animate({"bottom":"0px"});	//当前这张图片的标题运动出来
				});
